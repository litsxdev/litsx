...
import { createUseStateTransform } from "../packages/shared/babel-plugin-shared-hooks/src/index.js";
...
describe("@litsx/babel-plugin-shared-hooks createUseStateTransform", () => {
  it("rewrites useState calls to host-aware runtime state and injects prepareEffects", () => {
    ...
    assert.match(code, /import \{ useState, prepareEffects \} from "litsx";|import \{ prepareEffects, useState \} from "litsx";/);
  ...
  it("merges into an existing litsx runtime import instead of duplicating it", () => {
    ...
    assert.match(code, /import \{[^}]*useState[^}]*prepareEffects[^}]*useId[^}]*\} from ['"]litsx['"]|import \{[^}]*useId[^}]*useState[^}]*prepareEffects[^}]*\} from ['"]litsx['"]|import \{[^}]*prepareEffects[^}]*useId[^}]*useState[^}]*\} from ['"]litsx['"]/);
  });
