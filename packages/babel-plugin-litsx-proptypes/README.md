...
This plugin is part of the React-compat migration surface. Its job is to take React-style `Component.propTypes = { ... }` assignments and lower them into the internal property-metadata representation consumed by the rest of the LitSX pipeline.

## What it does

- Finds `Component.propTypes = { ... }` assignments on React-authored function components.
- Rewrites them to the internal `__litsx_static_properties({...})` representation.
- Preserves explicit authored `Component.properties = ...` metadata as the stronger override layer when both are present.
...
- Canonical output is the generated Lit class `static properties` contract.
