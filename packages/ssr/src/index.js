...
export function createDigestRewriter(mappings = globalThis[DIGEST_MAPPINGS]) {
  let pending = "";
...
export function rewriteRenderResult(result) {
  return transformResult(result, createDigestRewriter(), true);
}

export function render(value, renderInfo) {
  return rewriteRenderResult(renderLit(value, renderInfo));
}
